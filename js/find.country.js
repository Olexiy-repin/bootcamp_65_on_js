...
const onSearchElInput = ({ target: searchFieldEl }) => {
  const searchQuery = searchFieldEl.value.toLowerCase().trim();
  ...
  const country = countries.find(countryInfo => searchQuery === countryInfo.name.toLowerCase());

  if (country === undefined) {
    outputErrorEl.textContent = `${searchQuery} не знайдено!`;
    countryCardEl.innerHTML = '';
